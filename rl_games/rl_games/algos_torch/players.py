...
def _get_mixed_expl_num_blocks(config, fallback_envs):
    player_cfg = config.get('player', {}) or {}
    explicit_blocks = player_cfg.get('expl_num_blocks')
    if explicit_blocks is not None:
        return max(1, int(explicit_blocks))

    block_size = config.get('expl_coef_block_size')
    train_num_actors = config.get('num_actors')
    if block_size is not None and train_num_actors is not None:
        block_size = int(block_size)
        train_num_actors = int(train_num_actors)
        if block_size > 0 and train_num_actors >= block_size:
            return max(1, train_num_actors // block_size)

    return max(1, int(fallback_envs))
...
class PpoPlayerContinuous(BasePlayer):
    ...
    def __init__(self, params):
        ...
        if self.expl_type.startswith('mixed_expl'):
            num_blocks = _get_mixed_expl_num_blocks(self.config, self.env.num_envs)
            if 'disjoint' in self.expl_type or 'learn_param' in self.expl_type:
                ids = torch.linspace(50.0, 0.0, num_blocks, device=self.device).reshape(-1, 1)
            else:
                ids = create_sinusoidal_encoding(
                    torch.linspace(50.0, 0.0, num_blocks, device=self.device),
                    self.config.get('expl_reward_coef_embd_size', 32),
                    n=100,
                ).to(self.device)
            config['coef_ids'] = ids[:, 0]
