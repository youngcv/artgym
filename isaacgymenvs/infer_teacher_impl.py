...
def parse_args():
    parser = argparse.ArgumentParser(description="Run standalone teacher inference without rl_games play mode.")
    ...
    parser.add_argument("--expl-block-idx", type=int, default=0, help="Which teacher exploration block id to use during inference when applicable.")
    ...
    parser.add_argument("--save-latents", default="", help="Optional .pt path to save privileged latents.")
...
def main():
    ...
    if getattr(env_cls, "dict_obs_cls", False):
        raise RuntimeError("Teacher inference currently supports flat observations only.")
    ...
    if inferred_blocks is not None and player.intr_reward_coef_embd is not None:
        ...
        print(f"Using SAPG exploration block {block_idx} with id {coef_ids[block_idx].item():.4f}")
    ...
    if args.save_latents:
        ...
        print(f"Saved privileged latents to {latent_path}")
