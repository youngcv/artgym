...
def infer_student_temporal_obs_layout(student_obs_dim, history_len, proprio_dim_per_step, init_dim=DEFAULT_STUDENT_INIT_OBS_DIM):
    ...
    remaining_contact_flat_dim = student_obs_dim - base_layout["init_dim"] - (
        base_layout["history_len"] * base_layout["proprio_dim_per_step"]
    )
    if remaining_contact_flat_dim < 0:
        raise ValueError(
            "student_obs_dim is smaller than init_dim + history_len * proprio_dim_per_step: "
            f"student_obs_dim={student_obs_dim}, history_len={history_len}, "
            f"proprio_dim_per_step={proprio_dim_per_step}, init_dim={init_dim}"
        )

    if remaining_contact_flat_dim != 0:
        raise ValueError(
            "Tactile/contact student observations are no longer supported. "
            "Please re-distill the student with the current proprio-only observation layout. "
            f"Unexpected extra contact/history dims: {remaining_contact_flat_dim}."
        )

    return get_student_temporal_obs_layout(
        history_len=history_len,
        proprio_dim_per_step=proprio_dim_per_step,
        init_dim=init_dim,
    )
